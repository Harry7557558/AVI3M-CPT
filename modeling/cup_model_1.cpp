...
void triangulate_adaptive(ParamSurface F, double tol, int un, int vn) {
	std::vector<triangle_3d> T = AdaptiveParametricSurfaceTriangulator_dist(F).triangulate_adaptive(0., 2.*PI, 0., 1., un, vn, 16, tol, true, false);
	for (int i = 0; i < (int)T.size(); i++)
		trigs.push_back(triangle{ T[i][0], T[i][1], T[i][2], vec3(1.) });
}
...
vec3 map_lower(double u, double v) {
	vec2 c = mix(vec2(r0, 0), vec2(r0, r0_h2), v);
	vec3 s1 = vec3(c.x*cossin(u), c.y);
	vec3 s2 = smooth_fuse_G1(map_lower_0, map_lower_1, 0.2, 0.5, u, v);
	return s2;
	return mix(s2, s1, .6*(.5 - .5*tanh(200.*(.98 + cos(3.*u)))));
}
...
#if 1
#define triangulate triangulate_adaptive
#endif
	...
	if (0) {
		...
		int W, H;
		rgba *img = (rgba*)stbi_load("D:\\nautilus.jpg", &W, &H, nullptr, 3);
		printf("%d %d\n", W, H);
	...
	if (1) {
		...
		for (int i = 0; i < (int)trigs.size(); i++) {
			min_z = min(min(min_z, trigs[i].a.z), min(trigs[i].b.z, trigs[i].c.z));
		}
		for (int i = 0; i < (int)trigs.size(); i++) {
			trigs[i].a.z -= min_z, trigs[i].b.z -= min_z, trigs[i].c.z -= min_z;
		}
	}
